...
# systemd 单元同步：仓库 systemd/ 目录是唯一权威来源，
# 跟 /etc/systemd/system/ 里实际部署的内容不一致就覆盖过去。
# 不依赖"本次是否拉到新 commit"，每次运行都比对，保证最终收敛——
# 首次引入这段逻辑本身也是靠 git 分发的，board 拉到新版 update.sh 后
# 下一个 timer 周期就会执行到这里并自动补齐，不需要人工登录任何一台板子。
sync_systemd_units() {
    local repo_units="$REPO/systemd"
    local changed=0
    [ -d "$repo_units" ] || return 0
    for f in "$repo_units"/*; do
        local name dest
        name="$(basename "$f")"
        dest="/etc/systemd/system/$name"
        if ! sudo cmp -s "$f" "$dest" 2>/dev/null; then
            log "  同步 systemd 单元: $name"
            sudo cp "$f" "$dest"
            changed=1
        fi
    done
    if [ "$changed" = "1" ]; then
        log "  systemd 单元有变更，daemon-reload"
        sudo systemctl daemon-reload >>"$LOG" 2>&1
        sudo systemctl enable "$SERVICE" hdmi-setup.service >>"$LOG" 2>&1
        sudo systemctl enable --now lb-update.timer lb-backup-data.timer >>"$LOG" 2>&1
        # timer 的 OnCalendar/OnBootSec 改了要 restart 才会按新排期生效
        sudo systemctl restart lb-update.timer lb-backup-data.timer >>"$LOG" 2>&1
    fi
    return "$changed"
}
...
sync_systemd_units && log "  systemd 单元已是最新" || log "  systemd 单元已同步"
