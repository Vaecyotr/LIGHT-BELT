...
void test_scheduled_production_spi6_wire_times_cover_installed_lengths() {
  const uint16_t groups[] = {10, 20, 40};
  const uint32_t expected_bytes[] = {806, 986, 1346};
  const uint32_t expected_micros[] = {1290, 1578, 2154};
  for (size_t index = 0; index < 3; ++index) {
    const uint64_t encoded_len =
        light_belt::ws2811Spi6FrameSize(groups[index]);
    const uint64_t bit_micros = encoded_len * 8U * 1000000U;
    const uint32_t wire_time_us = static_cast<uint32_t>(
        (bit_micros + light_belt::WS2811_SPI6_CLOCK_HZ - 1U) /
        light_belt::WS2811_SPI6_CLOCK_HZ);
  ...
  TEST_ASSERT_GREATER_OR_EQUAL_UINT64(
      static_cast<uint64_t>(light_belt::WS2811_SPI6_CLOCK_HZ) *
          light_belt::WS2811_SPI6_RESET_LOW_US,
      static_cast<uint64_t>(light_belt::WS2811_SPI6_PRE_GUARD_BYTES) * 8U *
          1000000U);
  TEST_ASSERT_GREATER_OR_EQUAL_UINT64(
      static_cast<uint64_t>(light_belt::WS2811_SPI6_CLOCK_HZ) *
          light_belt::WS2811_SPI6_RESET_LOW_US,
      static_cast<uint64_t>(light_belt::WS2811_SPI6_POST_GUARD_BYTES) * 8U *
          1000000U);
}
...
int runTests() {
  ...
  RUN_TEST(test_scheduled_production_spi6_wire_times_cover_installed_lengths);
