...
constexpr uint32_t kPrimarySpiClockHz = WS2811_SPI6_CLOCK_HZ;
constexpr size_t kPrimarySpiMaxFrameBytes = WS2811_SPI6_MAX_FRAME_BYTES;
#else
...
bool SpiWs2811Backend::begin(
  ...
  if (kScheduledApplySupported) {
    ...
    const uint64_t required_guard_bit_micros =
        actual_clock > 0
        ? static_cast<uint64_t>(actual_clock) * WS2811_SPI6_RESET_LOW_US
        : 0;
...
SpiRefreshReport SpiWs2811Backend::prepare(const OwnedNodeFrame &frame) {
  ...
  if (pixelsUniform(output.pixels, output.descriptor.pixel_count)) {
    ...
    if (!ws2811Spi6UniformEncodedGroups(
