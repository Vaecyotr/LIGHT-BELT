...
.\.python\Scripts\python.exe -m light_engine `
  ...
  validate-show --show config/show.cabin-v2.yaml
...
`config/show.cabin-v2.yaml` is the writable Show v2 example. Its three authored
...
fragments, and protocol notes because they explain the development path. They
are not current production instructions. Files marked `STALE`, `legacy`, or
`historical` must not be copied into the cabin profile. No legacy file is
deleted by this phase: none was proven both unreferenced and redundant.
